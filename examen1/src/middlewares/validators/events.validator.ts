...
const validateDataToCreate = (req: Request, res: Response, next: NextFunction) => {
    const validations = [
        body('data.quote_id').isString().notEmpty().withMessage('Title is required'),
        body('data.time_toStart').isDate().notEmpty().withMessage('Date is required'),
        body('data.total_payed').isUUID().notEmpty().withMessage('place_id is required')
    ]
