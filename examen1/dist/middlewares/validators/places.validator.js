...
const validateData = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const validations = [
        ...
        (0, express_validator_1.body)('data.name').isString().withMessage('Name is required'),
        (0, express_validator_1.body)('data.description').isString().withMessage('Description is required'),
        (0, express_validator_1.body)('data.max_capacity').isNumeric().withMessage('Max capacity must be a number')
    ];
    yield Promise.all(validations.map(validation => validation.run(req)));
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
